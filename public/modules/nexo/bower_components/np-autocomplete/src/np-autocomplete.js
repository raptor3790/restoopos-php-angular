...
(function(root, factory) {
  ...
  .directive('npAutocomplete', ['$timeout', '$http', '$compile', '$templateCache', '$sce', function($timeout, $http, $compile, $templateCache, $sce) {
    ...
    $templateCache.put('np-autocomplete/template.tpl.html',
      ...
      '<div id="np-do-not-touch" style="display: none;position: absolute;">' +
      ' <button' +
      '   type="button"' +
      '   ng-click="clear()"' +
            ...
            timeoutId = $timeout(function() {
              ...
              $http.get(url, {
                ...
                if( typeof scope.options.callback == 'function' ) {
                    scope.options.callback( data, { close } );
                }

                if (scope.options.dataHolder) {
                  data = data[ scope.options.dataHolder ]; // eval('data.' + scope.options.dataHolder);
                }
